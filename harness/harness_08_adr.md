...
**배경 (Context)**
Phase A-3(전역 기술 컨벤션 확정) 단계에서, U1 작업지시서를 실제로 작성하기
전에 언어/프레임워크/DB가 정해져야 했음. 다수 지점 프랜차이즈(수천 명
규모) 웹 서비스이며, Claude Code 등 AI 에이전트의 구현 보조를 적극
활용할 계획.

**검토한 대안 (Options)**
| 대안 | 장점 | 단점 |
|---|---|---|
| A안: Next.js(React+TypeScript) + PostgreSQL + Prisma, 단일 배포 | 프론트/백엔드가 한 저장소·한 배포 단위라 운영이 단순. Prisma로 `branch_id` 필터링(ADR-001) 같은 데이터 접근 규칙을 코드 리뷰에서 확인하기 쉬움. AI 코드 생성 생태계 지원이 넓음 | 트래픽이 매우 커지면 API 라우트 확장 전략을 별도로 고민해야 함 |
| B안: Node.js/NestJS(백엔드) + React(프론트) 분리 + PostgreSQL | 역할별 권한 로직이 복잡한 이 프로젝트에 NestJS의 모듈/가드 구조가 유리할 수 있음 | 배포·구성 단계가 하나 더 늘어남(프론트/백엔드 별도 배포) |

**결정 (Decision)**
A안 채택 — Next.js(App Router, TypeScript) + PostgreSQL + Prisma.

**결과/트레이드오프 (Consequences)**
- 모든 API는 Next.js Route Handler(`/app/api/...`)로 구현.
- Prisma 스키마가 각 단위 TRD §1의 데이터 구조를 그대로 반영하는 단일
  진실 공급원이 된다.
- 대규모 트래픽 확장 전략(서버리스 vs 상시 서버)은 배포 시점(harness_09)에
  재검토 필요 — 지금은 결정하지 않음.
- 세부 컨벤션(인증 방식, API 응답 포맷, 디렉토리 구조 등)은
  `harness_13_tech_conventions.md`에서 이어서 확정.

**관련 문서**
관련 TRD: 전체 단위 공통. 관련 문서: `harness/harness_13_tech_conventions.md`

---

### ADR-005: 학생 개인정보 보유기간 및 파기 방식
- 날짜: 2026-09-06
- 상태: 승인됨
- 결정권자: 정찬성(프로젝트 오너)

**배경 (Context)**
U6(개인정보 생명주기) TRD 작성을 위해, 요구사항 정의 단계에서 잠정으로만
정해뒀던 "퇴원 후 1년 보관 후 자동 파기"의 정확한 일수를 최종 확정해야
했음. 개인정보 보유기간/파기는 원칙 8(되돌리기 어려운 결정) 대상이라
후보안 제시 후 사람 승인을 받음.

**검토한 대안 (Options)**
| 대안 | 장점 | 단점 |
|---|---|---|
| A안: 365일(1년) | 민원·분쟁 대응에 흔히 쓰이는 기간, 요구사항 정의 단계의 잠정값과 일치 | 6개월안보다 보유 리스크가 약간 더 김 |
| B안: 180일(6개월) | 개인정보 보유 리스크 최소화 | 분쟁 대응 기간이 짧아질 수 있음 |

**결정 (Decision)**
A안 채택 — 학생 `withdrawnAt` 기준 **365일** 경과 시 개인정보 파기(마스킹)
대상이 된다.

**결과/트레이드오프 (Consequences)**
- 파기 방식은 ADR-003의 연장선 — **행(row) 물리 삭제가 아니라 개인정보
  필드(name, birthDate, studentPhone, guardianPhone 등)를 마스킹**한다.
  Attendance·AuditLog 등 다른 테이블이 `studentId`로 이 학생을 참조하고
  있어 행 자체를 지우면 참조 무결성이 깨지기 때문.
- 마스킹 후에도 `Student` 행 자체와 `Attendance` 이력은 남아 통계·감사
  목적에 계속 쓰일 수 있다(단, 개인 식별 정보만 제거).
- 파기 배치 실행 이력은 U7 감사로그에 `student.purge` 이벤트로 기록한다.

**관련 문서**
관련 TRD: `docs/trd/att_u6_trd.md`. 관련 정책 문서: `harness/harness_10_data_lifecycle.md`
...
| ADR-004 | 핵심 기술 스택 선택 (Next.js+PostgreSQL+Prisma) | 승인됨 | 2026-09-06 |
| ADR-005 | 학생 개인정보 보유기간(365일) 및 파기(마스킹) 방식 | 승인됨 | 2026-09-06 |
