...
- 이 저장소는 `HARNESS_BASIC`(범용 하네스 템플릿)을 기반으로 생성된 **실제
  ...
  채워 나갑니다.
- 신규 개발 — 요구사항은 아직 확정 전
- 기준 브랜치: *(미확정 — 아래 "Git 워크플로우" 참고, 확정되는 대로 여기 기재)*

## 요구사항 / 완성 정의 (Definition of Done)
> 아직 정의되지 않았습니다. `harness/harness_00_definition.md` §4 Phase A
> 1단계(요구사항 + 비기능요구사항 정의)를 진행하면서 이 섹션을 채우세요.
> 확정 전까지는 어떤 기능 범위도 임의로 전제하지 않습니다.
