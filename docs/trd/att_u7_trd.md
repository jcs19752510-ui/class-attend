...
- 예외 상황:
  ...
  - 원장이 `branch_id` 파라미터로 타 지점을 지정 시도 → 서버가 무시하고
    자기 지점으로 강제(또는 403 처리 — §7에서 구현 시 택일).
...
| 403 FORBIDDEN_BRANCH | 타 지점 조회 시도 | 원장이 자기 지점 아닌 branch_id로 조회 시도(§7에서 구현 방식 확정) |
...
| 항목 | 권장 기본값 | 확정 필요 여부 |
|---|---|---|
| **개인정보 "조회"(view) 이벤트까지 로깅할 범위** — 요구사항 정의 ⑪에서 "개인정보 조회/수정 이력까지 기록"이라고 답변했으나, U2 TRD의 AC-6은 등록/수정/퇴원/배정변경만 다루고 "조회"는 포함하지 않음. 모든 GET 요청을 다 남기면 로그량·성능 부담이 커짐 | v0.1에서는 학생 **상세 단건 조회**(`GET /students/{id}`)만 `personal_data.view`로 로깅하고, 목록 조회는 제외 | **확정 필요 — 아래 별도 설명 참고, U2 TRD 소급 수정 여부도 함께 결정** |
| 원장이 `branch_id` 파라미터로 타 지점을 지정했을 때 "무시하고 자기 지점 강제" vs "403 반환" | 403 FORBIDDEN_BRANCH 반환(다른 단위와 일관된 에러 방식) | 확정 필요 |
| 감사로그 조회 행위 자체를 다시 로깅할지(메타 로깅) | 로깅 안 함(무한 재귀·로그 폭증 방지) | 확정 필요 |
| 감사로그의 보관기간(U6 개인정보 생명주기와 별개로 로그 자체도 영구보관인지) | 영구 보관(감사 목적, 학생/계정 데이터 파기와 무관하게 유지) | 확정 필요 |
