...
- 작성일 / 버전: 2026-09-05 (2026-09-06 AC-10 추가 개정) / v0.3
- 상태: 확정 (2026-09-06, U7 설계 중 발견된 조회 로깅 공백을 AC-10으로
  소급 반영 — harness_12 변경관리 참고 대상)
...
| `GET /students` | 필터(status 등) | 학생 목록 | 지점 내 학생 목록 (역할별 범위 적용). **목록 조회는 감사로그 대상 아님** |
| `GET /students/{id}` | - | 학생 상세 | 학생 상세 조회. **호출 시 `personal_data.view` 감사로그 1건 기록**(2026-09-06 확정) |
...
- [ ] AC-10: `GET /students/{id}` 호출 시마다 감사로그에
      `action_type=personal_data.view` 이벤트가 1건 기록되며(2026-09-06
      확정), `GET /students`(목록) 호출은 감사로그를 남기지 않는다.
...
| 학생 상세 조회 후 로그 확인 | 원장이 GET /students/{id} 호출 | 감사로그 1건(personal_data.view) 생성 | AC-10 |
| 학생 목록 조회 후 로그 확인 | 원장이 GET /students(목록) 호출 | 감사로그 생성 안 됨 | AC-10 |
